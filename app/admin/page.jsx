...
export function RequestCard({req}){
    return (
        <div className='flex items-center bg-blue-300 rounded-xl flex-row w-full p-2 '>
            ...
            <img className='rounded-full w-[50px] h-[50px]' src={req.image} alt="img"></img>
            <div className='flex flex-col pl-8'>
            <h1 className='text-black font-bold text-xs items-center'>External</h1>
            <h1 className='text-black text-2xl items-center'>{limit(req.name)}</h1>
            </div>
            </div>
            <button className='mr-8 bg-black text-white w-[100px] text-center rounded-xl'>View</button>
    ...
    const AddInstrument=async ()=>{
      ...
      try {
        const response = await fetch('/api/addinstruments',{
          method:"POST",
          body:JSON.stringify({"name":new_inst_name,"description":new_inst_desc,"image":new_inst_imagelink,"status":new_inst_status})
        });
        ...
        <div className='bg-black bg-opacity-25 h-[1000px] md:[800px]  lg:h-fit md:p-8 md:m-8 m-2 p-2 rounded-xl'>
            <div className='flex flex-col md:flex-row w-full md:h-full h-fit  z-10'>
                <div className='bg-white  flex flex-row md:flex-col h-fit md:h-full md:min-h-screen min-w-[20%]'>
                    <h1 className=' hidden md:flex px-2 items-center h-12 flex flex-row hover:border-blue-300 border-2 hover:bg-blue-100 text-xl text-blue-700'><i className="fa-brands fa-hive fa-3xl " style={{color:"black"}}></i>   <p className='ml-4 justify-center'>DashBoard</p></h1>
                    <h1 onClick={()=>setstate("stats")} className={`${state==="stats"?"bg-blue-200":"bg-white"} px-2 items-center h-12 flex flex-row hover:border-blue-300 border-2 hover:bg-blue-100 text-xl text-blue-700`}><i className="fa-solid fa-chart-simple fa-3xl " style={{color:"black"}}></i>   <button  className='ml-4 justify-center'>Stats</button></h1>
                    <h1 onClick={()=>setstate("instruments")} className={ `px-2 ${state==="instruments"?"bg-blue-200":"bg-white"} items-center h-12 flex flex-row hover:border-blue-300 border-2 hover:bg-blue-100 text-xl text-blue-700`}><i className="fa-solid fa-fax fa-3xl " style={{color:"black"}}></i>   <button  className='ml-4 justify-center'>Instruments</button></h1>
                    <h1 onClick={()=>setstate("cost")} className={`px-2 ${state==="cost"?"bg-blue-200":"bg-white"}  items-center h-12 flex flex-row hover:border-blue-300 border-2 hover:bg-blue-100 text-xl text-blue-700`}><i className="fa-solid fa-calculator fa-3xl " style={{color:"black"}}></i>   <button  className='ml-4 justify-center'>Cost</button></h1>
                    <h1 onClick={()=>setstate("request")} className={`px-2 ${state==="request"?"bg-blue-200":"bg-white"}  items-center h-12 flex flex-row hover:border-blue-300 border-2 hover:bg-blue-100 text-xl text-blue-700`}><i className="fa-solid fa-person-circle-question fa-3xl " style={{color:"black"}}></i>   <button  className='ml-4 justify-center'>Pending Request</button></h1>
                </div>
                    ...
                    <div className='bg-custompurple rounded-xl p-4 gap-y-2 flex flex-col w-full min-h-[400px] '>
                        <div className='flex flex-row  '>
                            ...
                            {peoplereq.length > 0 ? (
                                ...
                                <p className=" flex items-center justify-center h-full w-full ">Loading requests..</p>
                                )}   
                            </div>     
                    ...
                    <div className='bg-custompurple rounded-xl p-4 gap-y-2 flex flex-col w-full min-h-[400px]'>
                        <div className='flex flex-row  '>
                            ...
                            {instruments.length > 0 ? (
                                ...
                                <p className=" flex items-center justify-center h-full w-full ">Loading instruments..</p>
                                )}   
                            </div>     
                    ...
                    <div className=' bg-custompurple items-center justify-center flex col-span-2 w-full h-full'>
                        <div className='flex flex-row items-center gap-x-8 justify-center' >
                            <div className='flex flex-col bg-slate-300 rounded-xl  p-4'>
                                <h1 className='test-green-600 text-center'>Payment completed</h1>
            ...
            {state==="instruments" && (
              <div className='bg-custompurple rounded-xl  p-4 ml-2 flex flex-col w-full min-h-[600px] '>
                            ...
                            <>
                                  <Button className="h-[80px] my-4" onPress={onOpen}>Add Instruments</Button>
                                        ...
                                        <>
                                          ...
                                          <ModalBody>
                                            ...
                                            <Textarea
                                                ...
                                                className="max-w-xs"
                                                onChange={(e) => setnew_inst_imagelink(e.target.value)}
                                              />  
                                          ...
                                          <ModalFooter>
                                            ...
                                            <Button color="primary" onPress={AddInstrument}>ADD</Button>
                                            <Button ref={add_ins_button} className=" hidden" color="primary" onPress={onClose}>
                                              Action
                                            </Button>
                                          </ModalFooter>
                                        </>
